...
本手册对应 `main` 分支的 2026-09-05 精简基线。仓库只保存项目增量、测试、workload 与文档；
...
Icarus Verilog 和 Verilator；gem5、CoralNPU 与 Vortex 的额外系统依赖按各自上游说明安装。
...
make -C "$VORTEX_BUILD/sw/runtime" -j"$(nproc)"
make -C "$VORTEX_BUILD/tests/regression/vecadd" -j"$(nproc)"
```
